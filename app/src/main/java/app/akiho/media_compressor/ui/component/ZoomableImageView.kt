...
// https://engawapg.net/jetpack-compose/2237/zoom-image-3/

suspend fun PointerInputScope.detectTransformGestures(
    panZoomLock: Boolean = false,
    onGesture:
        (centroid: Offset, pan: Offset, zoom: Float, rotation: Float, timeMillis: Long) -> Boolean,
    onGestureStart: () -> Unit = {},
    onGestureEnd: () -> Unit = {},
) {
  forEachGesture {
...
class ZoomState(private val maxScale: Float) {
  private var _scale = Animatable(1f).apply { updateBounds(0.9f, maxScale) }
  val scale: Float
    get() = _scale.value
...
@Composable fun rememberZoomState(maxScale: Float) = remember { ZoomState(maxScale) }
...
fun ZoomableImageView(
  ...
  zoomState.setImageSize(Size.Zero)
  val scope = rememberCoroutineScope()
  LaunchedEffect(isVisible) { zoomState.reset() }
  ImageView(
      url = url,
      modifier =
                  ...
                  Modifier.fillMaxSize()
                      ...
                      .pointerInput(Unit) {
                        detectTransformGestures(
                            ...
                            onGestureEnd = { scope.launch { zoomState.endGesture() } })
                      }
                      .graphicsLayer {
